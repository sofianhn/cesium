...
describe(
  ...
  function () {
    ...
    it("loads tileset JSON file", async function () {
    ...
    it("loads tileset with extras", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("hasExtension returns true if the tileset JSON file uses the specified extension", async function () {
    ...
    it("passes version in query string to tiles", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("passes version in query string to all external resources", async function () {
      ...
      await Cesium3DTilesTester.loadTileset(
        scene,
    ...
    it("renders tileset", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    async function checkAnimation(url) {
      await Cesium3DTilesTester.loadTileset(scene, url);
      const renderOptions = {
        scene: scene,
    ...
    it("renders tileset in CV with projectTo2D option", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl, {
        projectTo2D: true,
      });
      scene.morphToColumbusView(0.0);
    ...
    it("renders tileset in 2D", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      scene.morphTo2D(0.0);
    ...
    it("renders tileset in 2D with projectTo2D option", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl, {
        projectTo2D: true,
      });
      scene.morphTo2D(0.0);
    ...
    it("does not render during morph", async function () {
      await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const commandList = scene.frameState.commandList;
      scene.renderForSpecs();
    ...
    it("renders tileset with empty root tile", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetEmptyRootUrl,
      );
    ...
    async function checkPointAndFeatureCounts(
      tileset,
      features,
      points,
      triangles,
    ...
    it("verify memory usage statistics for shared resources", async function () {
    ...
    it("does not process tileset when screen space error is not met", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const statistics = tileset._statistics;
      expect(statistics.visited).toEqual(5);
      expect(statistics.numberOfCommands).toEqual(5);
    ...
    it("does not select tiles when outside of view frustum", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const statistics = tileset._statistics;
      expect(statistics.visited).toEqual(5);
      expect(statistics.numberOfCommands).toEqual(5);
    ...
    it("does not load additive tiles that are out of view", async function () {
      viewBottomLeft();
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const statistics = tileset._statistics;
    ...
    it("culls with content box", async function () {
    ...
    it("selects children in front to back order", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("additive refinement - selects root when sse is met", async function () {
      viewRootOnly();
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("additive refinement - selects all tiles when sse is not met", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("additive refinement - use parent's geometric error on child's box for early refinement", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const statistics = tileset._statistics;
      expect(statistics.visited).toEqual(5);
      expect(statistics.numberOfCommands).toEqual(5);
    ...
    it("additive refinement - selects tile when inside viewer request volume", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
    ...
    it("replacement refinement - selects root when sse is met", async function () {
      viewRootOnly();
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("replacement refinement - selects children when sse is not met", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      tileset.root.refine = Cesium3DTileRefine.REPLACE;
    ...
    it("replacement refinement - selects root when sse is not met and children are not ready", async function () {
      viewRootOnly();
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const root = tileset.root;
    ...
    it("replacement refinement - selects tile when inside viewer request volume", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetWithViewerRequestVolumeUrl,
        {
    ...
    it("replacement refinement - selects root when sse is not met and subtree is not refinable (1)", async function () {
    ...
    it("replacement refinement - selects root when sse is not met and subtree is not refinable (2)", async function () {
    ...
    it("replacement refinement - refines if descendant is empty leaf tile", async function () {
      ...
      spyOn(Cesium3DTileset, "loadJson").and.callFake(
        async function (tilesetUrl) {
          const tilesetJson = await originalLoadJson(tilesetUrl);
    ...
    describe("children bound union optimization", function () {
      it("does not select visible tiles with invisible children", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("does not select external tileset whose root has invisible children", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetOfTilesetsUrl,
        );
      ...
      it("does not select visible tiles not meeting SSE with visible children", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetReplacementWithViewerRequestVolumeUrl,
        );
      ...
      it("does select visible tiles meeting SSE with visible children", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetReplacementWithViewerRequestVolumeUrl,
        );
        ...
        expect(isSelected(tileset_1, childRoot)).toBe(true);
      });

      it("does select visible tiles with visible children failing request volumes", async function () {
        viewRootOnly();
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetReplacementWithViewerRequestVolumeUrl,
      ...
      it("does select visible tiles with visible children passing request volumes", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetReplacementWithViewerRequestVolumeUrl,
        );
        ...
        expect(tileset_1._selectedTiles.length).toEqual(1);
        expect(isSelected(tileset_1, childRoot)).toBe(true);
        childRoot.geometricError = 200;
        scene.renderForSpecs();
        expect(tileset_1._selectedTiles.length).toEqual(4);
        expect(isSelected(tileset_1, childRoot)).toBe(false);
      });
    });
    ...
    it("renders tileset with external tileset JSON file", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetOfTilesetsUrl,
      );
    ...
    it("always visits external tileset root", async function () {
      viewRootOnly();
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetOfTilesetsUrl,
    ...
    it("set tile color", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        noBatchIdsUrl,
      );
    ...
    it("debugFreezeFrame", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      viewRootOnly();
      scene.renderForSpecs();
      const statistics = tileset._statistics;
    ...
    async function checkDebugColorizeTiles(url) {
    ...
    it("debugShowBoundingVolume", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      viewRootOnly();
    ...
    it("debugShowContentBoundingVolume", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      viewRootOnly();
    ...
    it("debugShowViewerRequestVolume", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetWithViewerRequestVolumeUrl,
      );
    ...
    it("show tile debug labels with regions", async function () {
    ...
    it("show tile debug labels with boxes", async function () {
    ...
    it("show tile debug labels with bounding spheres", async function () {
    ...
    it("show tile debug labels with rendering statistics", async function () {
      // tilesetUrl has bounding regions
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("show tile debug labels with memory usage", async function () {
      // tilesetUrl has bounding regions
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("show tile debug labels with all statistics", async function () {
      // tilesetUrl has bounding regions
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      tileset.debugShowGeometricError = true;
      tileset.debugShowRenderingStatistics = true;
    ...
    it("show only picked tile debug label with all stats", async function () {
      // tilesetUrl has bounding regions
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      tileset.debugShowGeometricError = true;
      tileset.debugShowRenderingStatistics = true;
    ...
    it("does not request tiles when picking", async function () {
      viewNothing();
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      viewRootOnly();
      scene.pickForSpecs();
    ...
    xit("does not request tiles when the request scheduler is full", async function () {
    ...
    it("load progress events are raised", async function () {
    ...
    it("tile visible event is raised", async function () {
      viewRootOnly();
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const spyUpdate = jasmine.createSpy("listener");
    ...
    it("tile load event is raised", async function () {
      viewNothing();
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const spyUpdate = jasmine.createSpy("listener");
    ...
    it("renders with imageBaseLightingFactor", async function () {
      const renderOptions = {
        scene: scene,
    ...
    async function testBackFaceCulling(url, setViewOptions) {
      const renderOptions = {
        scene: scene,
        time: new JulianDate(2457522.154792),
      };
    ...
    it("applies show style to a tileset", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withoutBatchTableUrl,
      );
    ...
    it("applies show style to a tileset without features", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        noBatchIdsUrl,
      );
    ...
    it("applies style with complex show expression to a tileset", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
    ...
    it("applies show style to a tileset with a composite tile", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        compositeUrl,
      );
    ...
    it("applies show style to a tileset with glTF content", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        gltfContentUrl,
      );
    ...
    it("applies show style to a tileset with glb content", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        glbContentUrl,
      );
    ...
    it("applies color style to a tileset", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withoutBatchTableUrl,
      );
    ...
    it("applies color style to a tileset with translucent tiles", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        translucentUrl,
      );
    ...
    it("applies color style to a tileset with translucent and opaque tiles", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
    ...
    it("applies color style to tileset without features", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        noBatchIdsUrl,
      );
    ...
    it("applies color style to tileset with glTF content", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        gltfContentUrl,
      );
    ...
    it("applies color style to tileset with glb content", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        glbContentUrl,
      );
    ...
    it("applies style when feature properties change", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
    ...
    it("applies style when tile is selected after new style is applied", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
    ...
    it("does not reapply style during pick pass", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
      ...
      scene.renderForSpecs();
      expect(
        tileset._statisticsPerPass[Cesium3DTilePass.RENDER].numberOfTilesStyled,
      ).toBe(1);
      scene.pickForSpecs();
      expect(
        tileset._statisticsPerPass[Cesium3DTilePass.PICK].numberOfTilesStyled,
      ).toBe(0);
    });

    it("applies style with complex color expression to a tileset", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
    ...
    it("applies conditional color style to a tileset", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
    ...
    it("handle else case when applying conditional color style to a tileset", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
    ...
    it("handle else case when applying conditional show to a tileset", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
    ...
    it("loads style from uri", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
      // ${id} < 10 will always evaluate to true
      const style = await Cesium3DTileStyle.fromUrl(styleUrl);
      tileset.style = style;
      expect(scene).toRenderAndCall(function (rgba) {
        expect(rgba[0]).toBeGreaterThan(0);
        expect(rgba[1]).toEqual(0);
    ...
    it("applies custom style to a tileset", async function () {
    ...
    it("applies style after show is toggled", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
    ...
    it("doesn't re-evaluate style during the next update", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
      ...
      const statistics = tileset._statisticsPerPass[Cesium3DTilePass.PRELOAD];
    ...
    it("doesn't re-evaluate style if the style being set is the same as the active style", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        withBatchTableUrl,
      );
      ...
      const statistics = tileset._statisticsPerPass[Cesium3DTilePass.RENDER];
      expect(statistics.numberOfTilesStyled).toBe(1);
    ...
    it("Unload all cached tiles not required to meet SSE using cacheBytes", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      tileset.cacheBytes = 0;
    ...
    it("Unload some cached tiles not required to meet SSE using cacheBytes", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("Restrict tileset memory usage with maximumCacheOverflowBytes", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      tileset.cacheBytes = 0.025 * 1024 * 1024; // Just enough memory to allow 3 tiles to remain
    ...
    it("Unloads cached tiles outside of the view frustum using cacheBytes", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      tileset.cacheBytes = 0;
    ...
    it("Unloads cached tiles in a tileset with external tileset JSON file using cacheBytes", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetOfTilesetsUrl,
      );
    ...
    it("Unloads cached tiles in a tileset with empty tiles using cacheBytes", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetEmptyRootUrl,
      );
    ...
    it("Unload cached tiles when a tileset uses replacement refinement using cacheBytes", async function () {
      // No children have content, but all grandchildren have content
      //
      //          C
      //      E       E
    ...
    it("Explicitly unloads cached tiles with trimLoadedTiles", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("tileUnload event is raised", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      tileset.cacheBytes = 0;
    ...
    it("propagates tile transform down the tree", async function () {
    ...
    it("does not mark tileset as refining when tiles have selection depth 0", async function () {
      viewRootOnly();
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetUrl,
    ...
    it("marks tileset as mixed when tiles have nonzero selection depth", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetReplacement3Url,
        skipLevelOfDetailOptions,
    ...
    it("adds stencil clear command first when unresolved", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetReplacement3Url,
        skipLevelOfDetailOptions,
    ...
    it("creates duplicate backface commands", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetReplacement3Url,
        skipLevelOfDetailOptions,
    ...
    it("does not create duplicate backface commands if no selected descendants", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetReplacement3Url,
        skipLevelOfDetailOptions,
    ...
    it("does not add stencil clear command or backface commands when fully resolved", async function () {
      viewAllTiles();
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetReplacement3Url,
    ...
    it("loadSiblings", async function () {
      viewBottomLeft();
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetReplacement3Url,
    ...
    it("immediatelyLoadDesiredLevelOfDetail", async function () {
      viewNothing();
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl, {
        skipLevelOfDetail: true,
    ...
    it("selects children if no ancestors available", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetOfTilesetsUrl,
        skipLevelOfDetailOptions,
    ...
    it("tile expires", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
    ...
    it("tile with tileset content expires", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
    ...
    it("tile expiration date", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const tile = tileset.root;
    ...
    it("supports content data URIs", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
    ...
    it("destroys attached ClippingPlaneCollections and ClippingPlaneCollections that have been detached", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("throws a DeveloperError when given a ClippingPlaneCollection attached to another Tileset", async function () {
      const tileset1 = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const clippingPlanes = new ClippingPlaneCollection({
        planes: [new ClippingPlane(Cartesian3.UNIT_X, 0.0)],
      });
      tileset1.clippingPlanes = clippingPlanes;

      const tileset2 = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      expect(function () {
        tileset2.clippingPlanes = clippingPlanes;
      }).toThrowDeveloperError();
    });

    it("clipping planes cull hidden tiles", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
    ...
    it("clipping planes cull tiles completely inside clipping region", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      const statistics = tileset._statistics;
      const root = tileset.root;
    ...
    it("clippingPlanesOriginMatrix has correct orientation", async function () {
      const tileset1 = await Cesium3DTilesTester.loadTileset(
        scene,
        withTransformBoxUrl,
      );
      ...
      let offsetMatrix = tileset1.clippingPlanesOriginMatrix;

      expect(
        Matrix4.equals(offsetMatrix, tileset1.root.computedTransform),
      ).toBe(true);

      const tileset2 = await Cesium3DTilesTester.loadTileset(scene, tilesetUrl);
      // The bounding volume of this tileset puts it on the surface,
      //  so we want to apply east-north-up as our best guess.
      offsetMatrix = tileset2.clippingPlanesOriginMatrix;
      expect(
        Matrix4.equals(offsetMatrix, tileset2.root.computedTransform),
      ).toBe(false);
      ...
      expect(
        Cartesian3.equals(
          tileset2.root.boundingSphere.center,
          clippingPlanesOrigin,
        ),
      ).toBe(true);
    });

    it("clippingPlanesOriginMatrix matches root tile bounding sphere", async function () {
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetOfTilesetsUrl,
      );
    ...
    it("allows setting the model matrix to its initial value when a tile contains a region", async function () {
      ...
      expect(function () {
        ...
        scene.renderForSpecs();
        expect(tileset.root.boundingVolume).toBeInstanceOf(TileBoundingRegion);
      }).not.toThrow();
    });
    ...
    describe("updateForPass", function () {
      it("updates for pass", async function () {
        viewAllTiles();
      ...
      it("uses custom command list", async function () {
    ...
    async function sampleHeightMostDetailed(cartographics, objectsToExclude) {
      let result;
      let completed = false;
      scene
    ...
    async function drillPickFromRayMostDetailed(ray, limit, objectsToExclude) {
      let result;
      let completed = false;
      scene
    ...
    describe("most detailed height queries", function () {
      it("tileset is offscreen", async function () {
        if (webglStub) {
          return;
        }
        ...
        expect(statisticsMostDetailedPick.numberOfTilesWithContentReady).toBe(
          1,
        );
      ...
      it("tileset is onscreen", async function () {
        if (webglStub) {
          return;
        }
      ...
      it("tileset uses additive refinement", async function () {
        if (webglStub) {
          return;
        }
        ...
        expect(statistics.numberOfTilesWithContentReady).toBeGreaterThanOrEqual(
          3,
        );
      ...
      it("drill picks multiple features when tileset uses additive refinement", async function () {
        if (webglStub) {
          return;
        }
        ...
        await Cesium3DTilesTester.loadTileset(scene, tilesetUniform);
        const results = await drillPickFromRayMostDetailed(ray);
        expect(results.length).toBe(3);
        expect(results[0].object.content.url.indexOf("0_0_0.b3dm") > -1).toBe(
          true,
        );
        expect(results[1].object.content.url.indexOf("1_1_1.b3dm") > -1).toBe(
          true,
        );
        expect(results[2].object.content.url.indexOf("2_4_4.b3dm") > -1).toBe(
          true,
        );
      });

      it("works when tileset cache is disabled", async function () {
        if (webglStub) {
          return;
        }
        viewNothing();
      ...
      it("multiple samples", async function () {
        if (webglStub) {
          return;
        }
    ...
    it("cancels out-of-view tiles", async function () {
      viewNothing();
      ...
      for (let i = 0; i < oldRequestsLength; i++) {
    ...
    it("defers requests when foveatedScreenSpaceError is true", async function () {
      viewNothing();
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetRefinementMix,
    ...
    it("loads deferred requests only after time delay.", async function () {
      viewNothing();
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetRefinementMix,
    ...
    it("preloads tiles", async function () {
      // Flight destination
      viewAllTiles();
    ...
    it("does not fetch tiles while camera is moving", async function () {
      viewNothing();
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetUniform,
    ...
    it("does not apply cullRequestWhileMoving optimization if tileset is moving", async function () {
      viewNothing();
      const tileset = await Cesium3DTilesTester.loadTileset(
        scene,
        tilesetUniform,
    ...
    it("loads tiles when preloadWhenHidden is true", async function () {
      const options = {
        show: false,
    ...
    it("displays copyrights for all glTF content", async function () {
      await Cesium3DTilesTester.loadTileset(scene, gltfContentWithCopyrightUrl);
      setZoom(10.0);
      scene.renderForSpecs();
      ...
      const credits = creditDisplay._currentFrameCredits.lightboxCredits.values;
    ...
    it("displays copyrights only for glTF content in view", async function () {
      await Cesium3DTilesTester.loadTileset(scene, gltfContentWithCopyrightUrl);
      const creditDisplay = scene.frameState.creditDisplay;
      ...
      expect(credits.values[0].credit.html).toEqual("Lower Right Copyright 1");
      expect(credits.values[1].credit.html).toEqual("Lower Right Copyright 2");
    ...
    it("displays copyrights for glTF content in sorted order", async function () {
      await Cesium3DTilesTester.loadTileset(
        scene,
    ...
    it("shows credits on screen", async function () {
      const options = {
    ...
    it("toggles showing credits on screen", async function () {
      const options = {
    ...
    describe("implicit tiling", function () {
      ...
      it("renders tileset with JSON subtree file", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("detects and initializes an implicit tileset in root tile", async function () {
        viewNothing();
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          implicitRootUrl,
      ...
      it("detects and initializes an implicit tileset in child tile", async function () {
        viewNothing();
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          implicitChildUrl,
        ...
        expect(
          endsWith(implicitTile._contentResource.url, "subtrees/0/0/0.subtree"),
        ).toEqual(true);
        expect(implicitTile.implicitTileset).toBeDefined();
        expect(implicitTile.implicitCoordinates).toBeDefined();
        expect(implicitTile.implicitCoordinates.level).toEqual(0);
      ...
      it("debugShowUrl works for implicit tiling", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          implicitTilesetUrl,
        );
    ...
    describe("3DTILES_implicit_tiling", function () {
      ...
      it("renders tileset (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("renders tileset with JSON subtree file (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("detects and initializes an implicit tileset in root tile (legacy)", async function () {
        viewNothing();
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("detects and initializes an implicit tileset in child tile (legacy)", async function () {
        viewNothing();
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("debugShowUrl works for implicit tiling (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          implicitTilesetLegacyUrl,
        );
    ...
    describe("multiple contents", function () {
      ...
      it("request statistics are updated correctly on success", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          multipleContentsUrl,
        );
      ...
      it("request statistics are updated correctly if requests are not scheduled", async function () {
        viewNothing();
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          multipleContentsUrl,
      ...
      it("renders tileset with multiple contents", async function () {
        const b3dmCommands = 1;
        // when instancing is supported, there is a single draw command,
        // else,each instance is a separate command.
        const i3dmCommands = scene.context.instancedArrays ? 1 : 25;
      ...
      it("renders implicit tileset with multiple contents", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
    ...
    describe("3DTILES_multiple_contents", function () {
      ...
      it("request statistics are updated correctly on success (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("request statistics are updated correctly if requests are not scheduled (legacy)", async function () {
        viewNothing();
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          multipleContentsLegacyUrl,
      ...
      it("renders tileset with multiple contents (legacy)", async function () {
        const b3dmCommands = 1;
        // when instancing is supported, there is a single draw command,
        // else,each instance is a separate command.
        const i3dmCommands = scene.context.instancedArrays ? 1 : 25;
      ...
      it("renders tileset with multiple contents (legacy with 'content')", async function () {
        const b3dmCommands = 1;
        // when instancing is supported, there is a single draw command,
        // else,each instance is a separate command.
        const i3dmCommands = scene.context.instancedArrays ? 1 : 25;
      ...
      it("renders implicit tileset with multiple contents (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("renders implicit tileset with multiple contents (legacy with 'content')", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
    ...
    describe("select feature IDs", function () {
      ...
      it("featureIdLabel sets from string", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("featureIdLabel sets from integer", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetWithFeatureIdsUrl,
        );
      ...
      it("instanceFeatureIdLabel sets from string", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetWithFeatureIdsUrl,
        );
      ...
      it("instanceFeatureIdLabel sets from integer", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetWithFeatureIdsUrl,
        );
    ...
    describe("metadata", function () {
      ...
      it("loads tileset metadata", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetMetadataUrl,
        );
      ...
      it("loads group metadata", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("can access group metadata through contents", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetWithGroupMetadataUrl,
        );
      ...
      it("loads metadata with embedded schema", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetMetadataUrl,
        );
      ...
      it("loads metadata with external schema", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("loads explicit tileset with tile metadata", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("loads implicit tileset with tile metadata", async function () {
      ...
      it("gracefully handles external tileset with tile metadata but no root schema", async function () {
      ...
      it("loads explicit tileset with content metadata", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
        ...
        tiles.forEach(function (tile) {
          ...
          expect(metadata.getProperty("author")).toEqual(expectedValues.author);
        });
      });

      it("loads implicit tileset with content metadata", async function () {
        // this tileset is similar to other implicit tilesets, though
        // one tile is removed
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
        ...
        for (let i = 0; i < tileCount; i++) {
          ...
          if (!subtree.contentIsAvailableAtIndex(index, 0)) {
            ...
            expect(metadata.getProperty("height")).toBe(expectedHeights[index]);
            expect(metadata.getProperty("color")).toEqual(
              expectedColors[index],
            );
          }
      ...
      it("gracefully handles external tileset with content metadata but no root schema", async function () {
      ...
      it("loads explicit tileset with multiple contents with metadata", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
        ...
        expect(batchedContentMetadata.getProperty("author")).toEqual("Cesium");
      ...
      it("loads implicit tileset with multiple contents with metadata", async function () {
        // this tileset is similar to other implicit tilesets, though
        // one tile is removed
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
    ...
    describe("3DTILES_metadata", function () {
      ...
      it("loads tileset metadata (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("loads group metadata (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
        ...
        expect(groups).toBeDefined();
        expect(groupIds).toEqual(["commercialDistrict", "residentialDistrict"]);
      ...
      it("can access group metadata through contents (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetWithGroupMetadataLegacyUrl,
        );
      ...
      it("loads metadata with embedded schema (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
          tilesetMetadataLegacyUrl,
        );
      ...
      it("loads metadata with external schema and extension (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("loads explicit tileset with tile metadata (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("loads implicit tileset with tile metadata (legacy)", async function () {
        // this tileset is similar to other implicit tilesets, though
        // one tile is removed
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("loads explicit tileset with content metadata (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
      ...
      it("loads explicit tileset with multiple contents with metadata (legacy)", async function () {
        const tileset = await Cesium3DTilesTester.loadTileset(
          scene,
